from __future__ import annotations

import re
from playwright.sync_api import Page, Locator
from utils.constants import SUPPLIERS_URL
from pages.common.form_page import has_required_field_feedback, has_validation_feedback


class SuppliersPage:
    def __init__(self, page: Page) -> None:
        self.page = page
        self.url = SUPPLIERS_URL

    def navigate(self) -> None:
        self.page.goto(self.url)
        self.page.wait_for_load_state("domcontentloaded")

    # ── Dynamic @property Locators ──────────────────────────────────────────

    @property
    def add_supplier_button(self) -> Locator:
        return self.page.get_by_role("button", name="Add Supplier")

    @property
    def modal_dialog(self) -> Locator:
        return self.page.get_by_role("dialog")

    @property
    def name_input(self) -> Locator:
        return self.modal_dialog.locator('input[name="name"]')

    @property
    def contact_person_input(self) -> Locator:
        return self.modal_dialog.locator('input[name="contact_person"]')

    @property
    def email_input(self) -> Locator:
        return self.modal_dialog.locator('input[name="email"]')

    @property
    def phone_input(self) -> Locator:
        return self.modal_dialog.get_by_role("textbox", name="Enter 10 digit phone number")

    @property
    def gst_input(self) -> Locator:
        return self.modal_dialog.get_by_role("textbox", name="Enter 15-character GST number")

    @property
    def state_container(self) -> Locator:
        return self.modal_dialog.locator("input[name='state_id']").locator("..")

    @property
    def city_container(self) -> Locator:
        return self.modal_dialog.locator("input[name='city_id']").locator("..")

    @property
    def postal_code_input(self) -> Locator:
        return self.modal_dialog.locator('input[name="postal_code"]')

    @property
    def address_input(self) -> Locator:
        return self.modal_dialog.locator('textarea[name="address"]')

    @property
    def notes_button(self) -> Locator:
        return self.modal_dialog.get_by_role("button", name="Add Notes")

    @property
    def notes_input(self) -> Locator:
        return self.modal_dialog.locator('textarea[name="notes"]')

    @property
    def account_number_input(self) -> Locator:
        return self.modal_dialog.locator('input[name="account_number"]')

    @property
    def ifsc_input(self) -> Locator:
        return self.modal_dialog.locator('input[name="ifsc"]')

    @property
    def create_button(self) -> Locator:
        return self.modal_dialog.get_by_role("button", name="Create")

    @property
    def update_button(self) -> Locator:
        return self.modal_dialog.get_by_role("button", name="Update")

    @property
    def search_input(self) -> Locator:
        return self.page.get_by_role("textbox", name="Search supplier...")

    def is_suppliers_visible(self) -> bool:
        return self.add_supplier_button.is_visible() or self.search_input.is_visible()

    def _select_option(self, container: Locator, option_name: str) -> None:
        container.wait_for(state="visible", timeout=5000)
        container.click()

        search_input = container.locator("input[id^='react-select'], input[type='text']")
        if search_input.count() > 0 and search_input.first.is_visible():
            search_input.first.fill(option_name)

        opt = self.page.get_by_role("option", name=option_name)
        if opt.count() > 0:
            try:
                opt.first.wait_for(state="visible", timeout=3000)
                opt.first.click()
                return
            except Exception:
                pass

        matching_option = self.page.locator(".react-select__option, div[class*='-option']").filter(has_text=option_name)
        if matching_option.count() > 0:
            matching_option.first.wait_for(state="visible", timeout=3000)
            matching_option.first.click()
        elif search_input.count() > 0 and search_input.first.is_visible():
            search_input.first.press("Enter")
        else:
            first_opt = self.page.locator(".react-select__option, div[class*='-option']").first
            try:
                first_opt.wait_for(state="visible", timeout=3000)
                first_opt.click()
            except Exception:
                pass

    def _fill_supplier_form(
        self,
        name: str,
        contact_person: str,
        email: str,
        phone: str,
        gst_number: str,
        state_name: str,
        city_name: str,
        postal_code: str,
        address: str,
        notes: str | None = None,
        account_number: str | None = None,
        ifsc: str | None = None,
    ) -> None:
        self.name_input.fill(name)
        self.contact_person_input.fill(contact_person)
        self.email_input.fill(email)
        self.phone_input.fill(phone)
        self.gst_input.fill(gst_number)

        self._select_option(self.state_container, state_name)
        self.city_container.wait_for(state="visible", timeout=5000)
        self._select_option(self.city_container, city_name)

        self.postal_code_input.fill(postal_code)
        self.address_input.fill(address)

        if notes:
            try:
                if self.notes_button.is_visible():
                    self.notes_button.click()
            except Exception:
                pass
            if self.notes_input.is_visible():
                self.notes_input.fill(notes)

        if account_number and self.account_number_input.is_visible():
            self.account_number_input.fill(account_number)

        if ifsc and self.ifsc_input.is_visible():
            self.ifsc_input.fill(ifsc)

    def add_supplier(
        self,
        name: str,
        contact_person: str = "Auto Contact",
        email: str | None = None,
        phone: str | None = None,
        gst_number: str | None = None,
        state_name: str = "Tamil Nadu",
        city_name: str | None = None,
        postal_code: str | None = None,
        address: str = "123 Auto Street",
        notes: str | None = "automation test notes",
        account_number: str | None = None,
        ifsc: str | None = None,
    ) -> str:
        from pages.master_menu.cities_page import CitiesPage
        from utils.random_data import (
            generate_random_code, generate_random_email, generate_random_phone,
            generate_random_postal_code, generate_random_gst
        )

        if not city_name:
            cities_page = CitiesPage(self.page)
            if hasattr(self, "url") and self.url:
                base = self.url.split("/suppliers")[0]
                cities_page.city_url = f"{base}/cities"
            cities_page.navigate()
            city_name = f"AutoCity_{generate_random_code('S')}"
            cities_page.add_city(city_name)

        self.navigate()
        email = email or generate_random_email("supplier")
        phone = phone or generate_random_phone()
        postal_code = postal_code or generate_random_postal_code()
        gst_number = gst_number or generate_random_gst()

        self.add_supplier_button.click()
        modal = self.modal_dialog
        modal.wait_for(state="visible", timeout=10000)

        self._fill_supplier_form(
            name, contact_person, email, phone, gst_number, state_name,
            city_name, postal_code, address, notes, account_number, ifsc
        )

        try:
            with self.page.expect_response(
                lambda r: "supplier" in r.url.lower() and r.request.method == "POST",
                timeout=10000
            ) as resp_info:
                self.create_button.click()
            assert resp_info.value.status in (200, 201), f"Supplier creation failed with HTTP {resp_info.value.status}"
        except Exception:
            if self.create_button.is_visible():
                self.create_button.click()

        toast = self.page.get_by_text(re.compile(r"Suppliers created|Supplier created|Operation successful|success", re.IGNORECASE)).first
        try:
            toast.wait_for(state="visible", timeout=5000)
        except Exception:
            pass
        modal.wait_for(state="hidden", timeout=10000)
        return city_name

    def _ensure_modal_closed(self) -> None:
        if not self.modal_dialog.is_visible():
            return
        close_btn = self.modal_dialog.locator(
            ".btn-close, button:has-text('Cancel'), button:has-text('Close')"
        ).first
        try:
            if close_btn.is_visible():
                close_btn.click(timeout=3000)
        except Exception:
            if self.modal_dialog.is_visible():
                self.page.keyboard.press("Escape")
        self.modal_dialog.wait_for(state="hidden", timeout=5000)

    def search_supplier(self, name: str) -> bool:
        self.search_input.fill(name)
        self.search_input.press("Enter")
        row = self.page.locator("table tbody tr").filter(has=self.page.get_by_text(name, exact=True))
        try:
            row.first.wait_for(state="visible", timeout=5000)
            return True
        except Exception:
            return False

    def is_supplier_active(self, name: str) -> bool:
        return self.search_supplier(name)

    def view_supplier(
        self,
        name: str,
        expected_contact: str | None = None,
        expected_email: str | None = None,
    ) -> bool:
        self.search_supplier(name)
        row = self.page.locator("table tbody tr").filter(has=self.page.get_by_text(name, exact=True))
        row.first.wait_for(state="visible", timeout=5000)

        row.first.locator("button[title='view'], a[title='view']").first.click()
        modal = self.modal_dialog
        modal.wait_for(state="visible", timeout=5000)

        try:
            modal.get_by_text(name, exact=True).first.wait_for(state="visible", timeout=5000)
            modal_text = modal.inner_text()
            is_visible = name in modal_text
            if expected_contact:
                is_visible = is_visible and expected_contact in modal_text
            if expected_email:
                is_visible = is_visible and expected_email in modal_text
        except Exception:
            is_visible = False

        modal.get_by_role("button", name="Back to List").click()
        modal.wait_for(state="hidden", timeout=5000)
        return is_visible

    def edit_supplier(
        self,
        old_name: str,
        new_name: str,
        new_contact: str | None = None,
        new_email: str | None = None,
    ) -> bool:
        self.search_supplier(old_name)
        row = self.page.locator("table tbody tr").filter(has=self.page.get_by_text(old_name, exact=True))
        row.first.wait_for(state="visible", timeout=5000)

        row.first.locator("button[title='edit'], a[title='edit']").first.click()
        modal = self.modal_dialog
        modal.wait_for(state="visible", timeout=5000)

        self.name_input.fill(new_name)
        if new_contact:
            self.contact_person_input.fill(new_contact)
        if new_email:
            self.email_input.fill(new_email)

        with self.page.expect_response(
            lambda r: "supplier" in r.url.lower() and r.request.method in ("PUT", "PATCH"),
            timeout=10000
        ) as resp_info:
            self.update_button.click()

        assert resp_info.value.status in (200, 204), f"Edit supplier failed with HTTP {resp_info.value.status}"
        modal.wait_for(state="hidden", timeout=10000)
        return True

    def delete_supplier(self, name: str) -> bool:
        if not self.search_supplier(name):
            return False
        row = self.page.locator("table tbody tr").filter(has=self.page.get_by_text(name, exact=True))
        row.first.wait_for(state="visible", timeout=5000)

        active_delete_btn = row.first.locator("button[title='delete']:has(i.bi-trash)")
        if active_delete_btn.count() == 0:
            return False
        active_delete_btn.first.click()

        modal = self.modal_dialog
        modal.wait_for(state="visible", timeout=5000)

        delete_btn = modal.get_by_role("button", name="Delete Supplier")
        try:
            delete_btn.wait_for(state="visible", timeout=3000)
            with self.page.expect_response(
                lambda r: "supplier" in r.url.lower() and r.request.method == "DELETE",
                timeout=10000
            ) as resp_info:
                delete_btn.click()
            assert resp_info.value.status in (200, 204), f"Delete supplier failed with HTTP {resp_info.value.status}"
        except Exception:
            close_btn = modal.locator(".btn-close")
            if close_btn.is_visible():
                close_btn.click()
            elif modal.get_by_role("button", name="Cancel").is_visible():
                modal.get_by_role("button", name="Cancel").click()
            return False

        modal.wait_for(state="hidden", timeout=5000)
        return True

    def retrieve_supplier(self, name: str) -> bool:
        self.search_supplier(name)
        row = self.page.locator("table tbody tr").filter(has=self.page.get_by_text(name, exact=True))
        row.first.wait_for(state="visible", timeout=5000)

        # Rule 3: Wait for i.bi-arrow-clockwise on soft-deleted row before retrieve
        restore_btn = row.first.locator("button[title='delete']:has(i.bi-arrow-clockwise)")
        restore_btn.wait_for(state="visible", timeout=5000)
        restore_btn.click()

        modal = self.modal_dialog
        modal.wait_for(state="visible", timeout=5000)

        retrieve_btn = modal.get_by_role("button", name="Retrieve Supplier")
        retrieve_btn.wait_for(state="visible", timeout=3000)
        retrieve_btn.click()

        modal.wait_for(state="hidden", timeout=5000)
        return True

    def validate_invalid_field(
        self,
        name: str,
        contact_person: str,
        email: str,
        phone: str,
        gst_number: str,
        state_name: str,
        city_name: str,
        postal_code: str,
        address: str,
        field: str,
        value: str,
        notes: str | None = None,
        account_number: str | None = None,
        ifsc: str | None = None,
    ) -> bool:
        self.navigate()
        self._ensure_modal_closed()
        self.add_supplier_button.click()
        modal = self.modal_dialog
        modal.wait_for(state="visible", timeout=10000)

        args = {
            "name": name, "contact_person": contact_person, "email": email, "phone": phone,
            "gst_number": gst_number, "state_name": state_name, "city_name": city_name,
            "postal_code": postal_code, "address": address, "notes": notes,
            "account_number": account_number, "ifsc": ifsc
        }
        args[field] = value
        self._fill_supplier_form(**args)

        api_rejected = False
        try:
            with self.page.expect_response(
                lambda r: "supplier" in r.url.lower() and r.request.method == "POST",
                timeout=3000
            ) as resp_info:
                self.create_button.click()
            api_rejected = resp_info.value.status in (400, 422)
        except Exception:
            api_rejected = True

        patterns = {
            "email": (r"email.*(?:invalid|valid|format|domain)", r"invalid.*email", r"already.*taken", r"validation"),
            "phone": (r"phone.*(?:invalid|valid|10|digits|start)", r"invalid.*phone", r"validation"),
            "gst_number": (r"gst.*(?:invalid|valid|format|15|characters)", r"invalid.*gst", r"validation"),
            "postal_code": (r"postal.*(?:invalid|valid|6|digits)", r"pincode.*(?:invalid|valid|6|digits)", r"validation"),
            "account_number": (r"account.*(?:invalid|valid|number|digits)", r"validation"),
            "ifsc": (r"ifsc.*(?:invalid|valid|code|format)", r"validation"),
        }
        ui_rejected = has_validation_feedback(modal, *patterns[field])
        self._ensure_modal_closed()
        return api_rejected and ui_rejected

    def validate_duplicate_supplier(
        self,
        name: str,
        contact_person: str,
        email: str,
        phone: str,
        gst_number: str,
        state_name: str,
        city_name: str,
        postal_code: str,
        address: str,
        notes: str | None = None,
    ) -> bool:
        self.navigate()
        self._ensure_modal_closed()
        self.add_supplier_button.click()
        modal = self.modal_dialog
        modal.wait_for(state="visible", timeout=10000)

        self._fill_supplier_form(
            name, contact_person, email, phone, gst_number, state_name,
            city_name, postal_code, address, notes
        )

        api_rejected = False
        try:
            with self.page.expect_response(
                lambda r: "supplier" in r.url.lower() and r.request.method == "POST",
                timeout=5000
            ) as resp_info:
                self.create_button.click()
            api_rejected = resp_info.value.status in (400, 422)
        except Exception:
            api_rejected = True

        ui_rejected = has_validation_feedback(
            modal,
            r"supplier.*already",
            r"(?:email|phone|gst).*(?:already|duplicate|taken|exist)",
            r"duplicate.*supplier",
            r"already.*taken",
            r"validation",
        )
        self._ensure_modal_closed()
        return api_rejected and ui_rejected

    def validate_required_fields(self) -> bool:
        self.navigate()
        self._ensure_modal_closed()
        self.add_supplier_button.click()
        modal = self.modal_dialog
        modal.wait_for(state="visible", timeout=10000)

        api_rejected = False
        try:
            with self.page.expect_response(
                lambda r: "supplier" in r.url.lower() and r.request.method == "POST",
                timeout=3000
            ) as resp_info:
                self.create_button.click()
            api_rejected = resp_info.value.status in (400, 422)
        except Exception:
            api_rejected = True

        ui_rejected = has_required_field_feedback(modal)
        self._ensure_modal_closed()
        return api_rejected and ui_rejected
