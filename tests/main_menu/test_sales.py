...
def module_category(module_page):
    ...
    cat_name = generate_random_name("s_cat")
...
def module_brand(module_page):
    ...
    brand_page.navigate()
    brand_name = generate_random_name("s_brand")
...
def module_unit_type(module_page):
    ...
    unit_page.navigate()
    unit_name = generate_random_name("s_unit")
...
def module_city(module_page):
    ...
    city_name = generate_random_name("s_city")
...
def module_customer(module_page, module_city):
    ...
    customer_name = generate_random_name("s_cust")
...
def module_product(module_page, module_category, module_brand, module_unit_type, module_hsn_code, module_branch):
    ...
    product_name = generate_random_name("s_prod")
...
def test_sales_crud_flow(
    ...
    # 3. Filter Sales List
    sales_page.filter_sales(salesperson_name="Super Admin", branch_name=module_branch)

    # 4. View Sale details
    assert sales_page.view_sale(
        query=module_customer,
        branch_name=module_branch,
        salesperson_name="Super Admin"
    )

    # 5. Edit Sale quantity to 2
    sales_page.edit_sale(query=module_customer, new_quantity=2)

    # 6. Download Invoice and verify
    download_path = sales_page.download_invoice(query=module_customer)
    assert download_path and os.path.exists(download_path), "Invoice download failed"
    assert os.path.getsize(download_path) > 0, "Downloaded invoice is empty"
