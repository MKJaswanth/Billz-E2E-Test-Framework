"""Restaurant GSTR-1 B2B Report Test Suite.
...
from pages.Verticals.Restaurant.report.gstr_1_b2b_page import Gstr1B2bPage
...
def _assert_contract(data: dict, mode: str = "invoice_wise") -> list[dict]:
    assert isinstance(data, dict), "B2B report did not return a valid data dictionary"
...
class TestResGstr1B2bStructure:
    """Verify page load and API contract."""

    def test_gstr1_b2b_page_loads_real_report(self, res_logged_in_page):
        page = res_logged_in_page
        report = Gstr1B2bPage(page)
        data = report.navigate()

        assert report.heading_visible(), "GSTR-1 B2B heading should be visible"
...
class TestResGstr1B2bCalculations:
    """Verify tax calculations and totals reconciliation."""

    def test_b2b_totals_equal_rendered_invoice_rows(self, res_logged_in_page):
        page = res_logged_in_page
        report = Gstr1B2bPage(page)
        data = report.navigate()
        ...
        assert int(totals.get("invoice_count", 0)) == len(rows)
        for key in ("taxable_value", "cgst_amount", "sgst_amount", "igst_amount"):
            if key in totals:
                expected_sum = sum((_money(row[key]) for row in rows if key in row), Decimal("0.00"))
                assert _money(totals[key]) == expected_sum, (
                    f"Mismatch in total {key}: expected {expected_sum}, got {totals[key]}"
                )
    ...
    def test_invoice_rows_tax_calculation_integrity(self, res_logged_in_page):
        ...
        rows = _assert_contract(data)
        if not rows:
            pytest.skip("No B2B invoices present to verify individual row tax calculations")

        for row in rows:
            assert row.get("classification") == "b2b"
...
class TestResGstr1B2bFilters:
    """Verify date filtering and summary/invoice view modes."""

    def test_b2b_custom_date_filter_sends_exact_parameters(self, res_logged_in_page):
        page = res_logged_in_page
        report = Gstr1B2bPage(page)
        report.navigate()
    ...
    def test_b2b_summary_mode_switch(self, res_logged_in_page):
        page = res_logged_in_page
        report = Gstr1B2bPage(page)
        report.navigate()
